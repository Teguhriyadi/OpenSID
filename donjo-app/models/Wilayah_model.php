...
class Wilayah_model extends MY_Model
{
    ...
    {
        $this->db->select("MIN(`tweb_wil_clusterdesa`.`id`) as id, `tweb_wil_clusterdesa`.`dusun`, `penduduk_hidup`.`nama` as `nama`, `tweb_wil_clusterdesa`.`rw`, `tweb_wil_clusterdesa`.`rt` ", FALSE);
        $this->db->from("tweb_wil_clusterdesa");
        $this->db->join("penduduk_hidup", "tweb_wil_clusterdesa.id_kepala = penduduk_hidup.id");
        
        if (!empty($postData["search"]["value"])) {
            $this->db->like("tweb_wil_clusterdesa.id", $postData["search"]["value"]);
        }
        
        $this->db->group_by("tweb_wil_clusterdesa.dusun, penduduk_hidup.nama");
        $this->db->order_by("MIN(tweb_wil_clusterdesa.id)", "DESC");
